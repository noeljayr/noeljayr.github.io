...
const titleInput = document.getElementById("postId");
...
if (stringAfterUnderscore) {
  ...
  for (const post of posts) {
    if (post.ID === parseInt(stringAfterUnderscore)) {
      titleInput.value = post.post_title;
      contentInput.innerHTML = post.post_content;
      formSelect.value = post.category.trim();
      var event = new Event("change");

      // Dispatch the event on the element
      formSelect.dispatchEvent(event);
      options.forEach((o) => {
        if (o.textContent == post.category.trim()) {
      ...
      selectedT.textContent = post.category.trim();
...
function addPlatform() {
  if (platFormName.value.length > 0 && plaformLinkInput.value.length > 0) {
    ...
    let platformIcon = checkPlatformIcon(platform.name);
